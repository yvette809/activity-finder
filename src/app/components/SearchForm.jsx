...
const SearchForm = ({ onSearch, activities }) => {
  ...
  useEffect(() => {
    ...
    // Fetch creator data based on unique IDs if needed

    // Set the unique creators in the state
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
          <div className="flex-1">
            <label className="block mb-2 font-medium">Type of activity:</label>
            <select
              name="typeOfActivity"
              onChange={(event) =>
        ...
        <button
          type="submit"
          className="w-full bg-deep-green text-white p-2 rounded-md hover:bg-blue-700 transition duration-300"
        >
          Search
        </button>
      </form>
